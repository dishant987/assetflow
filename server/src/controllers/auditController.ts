...
export async function getCycleById(req: Request, res: Response) {
  const data = await service.getCycleById(Number(req.params.id));
  res.json({ data });
}
...
export async function populateItems(req: Request, res: Response) {
  const count = await service.populateItems(Number(req.params.id));
  res.json({ data: { count } });
}

export async function startCycle(req: Request, res: Response) {
  const data = await service.updateCycleStatus(Number(req.params.id), "in_progress");
  res.json({ data });
}

export async function completeCycle(req: Request, res: Response) {
  const data = await service.updateCycleStatus(Number(req.params.id), "completed");
  res.json({ data });
}

export async function cancelCycle(req: Request, res: Response) {
  const data = await service.updateCycleStatus(Number(req.params.id), "cancelled");
  res.json({ data });
}
